...
# Tree listing with directory/file count
...
lt() {
    ...
    eza --tree --level="$level" --icons --group-directories-first

    local dirs files
    dirs=$(find . -mindepth 1 -maxdepth "$level" \
        ! -path '*/.*' -type d 2>/dev/null | wc -l)

    files=$(find . -mindepth 1 -maxdepth "$level" \
        ! -path '*/.*' ! -type d 2>/dev/null | wc -l)
...
# opencode
export PATH=/home/aakash/.opencode/bin:$PATH

# kilo
export PATH=/home/aakash/.kilo/bin:$PATH
